...
for binding in "${UE_BINDINGS[@]}"; do
  ...
  if [[ -z "$tunnel_ip" ]]; then
    echo "[sensor-traffic] ERROR: missing oaitun_ue1 ue_id=${ue_label} container=${container_name}" >&2
    exit 1
  fi
  log_path="${OUTPUT_DIR}/${log_file_name}"
  echo "[sensor-traffic] START ue_id=${ue_label} container=${container_name} tunnel_interface=oaitun_ue1 tunnel_ip=${tunnel_ip} target_ip=${TARGET_IP} target_port=${TARGET_PORT} periodic UDP (${log_path})"
  run_udp_sender \
    "$container_name" \
    "sensor_telemetry" \
    "periodic_udp" \
